...
class LogUtils private constructor() {
    ...
    class Config internal constructor() {
        internal var mDefaultDir: String? = null// The default storage directory of log.
        internal var mDir: String? = null       // The storage directory of log.
        internal var mFilePrefix = "util"// The file prefix of log.
        internal var mLogSwitch = true  // The switch of log.
        internal var mLog2ConsoleSwitch = true  // The logcat's switch of log.
        internal var mGlobalTag: String? = null  // The global tag of log.
        internal var mTagIsSpace = true  // The global tag is space.
        internal var mLogHeadSwitch = true  // The head's switch of log.
        internal var mLog2FileSwitch = false // The file's switch of log.
        internal var mLogBorderSwitch = true  // The border's switch of log.
        internal var mSingleTagSwitch = true  // The single tag of log.
        internal var mConsoleFilter = V     // The console's filter of log.
        internal var mFileFilter = V     // The file's filter of log.
        internal var mStackDeep = 1     // The stack's deep of log.
        internal var mStackOffset = 0     // The stack's offset of log.

        init {
            if (mDefaultDir == null) {
                mDefaultDir =
                        if (Environment.MEDIA_MOUNTED == Environment.getExternalStorageState()
                                && App.instance.externalCacheDir != null)
                            App.instance.externalCacheDir.toString() + FILE_SEP + "log" + FILE_SEP
                        else {
                            App.instance.cacheDir.toString() + FILE_SEP + "log" + FILE_SEP
                        }
            }
        }
        ...
        fun setFilePrefix(filePrefix: String): Config {
            mFilePrefix = (if (isSpace(filePrefix)) "util" else filePrefix)
            return this
        }
    ...
    companion object {
        ...
        private fun processTagAndHead(tag: String?): TagHead {
            ...
            if (!config.mTagIsSpace && !config.mLogHeadSwitch) {
                ...
                if (stackIndex >= stackTrace.size) {
                    ...
                    if (config.mTagIsSpace && isSpace(tag_)) {
                        val index = fileName.indexOf('.')// Use proguard may not find '.'.
                        tag_ = if (index == -1) fileName else fileName.substring(0, index)
                    }
                    return TagHead(tag_, null, ": ")
                }
                ...
                if (config.mLogHeadSwitch) {
                    ...
                    if (config.mStackDeep <= 1) {
                        return TagHead(tag_, arrayOf(head), fileHead)
                    } else {
                        ...
                        return TagHead(tag_, consoleHead, fileHead)
                    }
                }
            }
            return TagHead(tag_, null, ": ")
        ...
        private fun formatJson(json: String): String {
            ...
            try {
                if (json_.startsWith("{")) {
                    json_ = JSONObject(json_).toString(4)
                } else if (json_.startsWith("[")) {
                    json_ = JSONArray(json_).toString(4)
                }
        ...
        private fun formatXml(xml: String): String {
            var xml_ = xml
            try {
                val xmlInput = StreamSource(StringReader(xml_))
                val xmlOutput = StreamResult(StringWriter())
                val transformer = TransformerFactory.newInstance().newTransformer()
                transformer.setOutputProperty(OutputKeys.INDENT, "yes")
                transformer.setOutputProperty("{http://xml.apache.org/xslt}indent-amount", "4")
                transformer.transform(xmlInput, xmlOutput)
                xml_ = xmlOutput.writer.toString().replaceFirst(">".toRegex(), ">$LINE_SEP")
            } catch (e: Exception) {
                e.printStackTrace()
            }
